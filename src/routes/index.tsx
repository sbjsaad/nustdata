...
function PageIntro({ active }: { active: string }) {
  const reports = active === 'Reports';
  return <div className="mb-7 grid grid-cols-[minmax(0,1fr)_auto] items-end gap-4"><div className="min-w-0"><p className="text-sm font-semibold text-primary">Academic Year 2025–26</p><h1 className="mt-1 truncate font-display text-3xl font-bold tracking-tight">{reports ? 'Billing reports' : 'Data entry workspace'}</h1><p className="mt-1 text-sm text-muted-foreground">{reports ? 'Review, filter, and read boarder billing records.' : 'Enter and organize boarder billing information.'}</p></div>{!reports && <Button variant="enterprise" className="shrink-0"><FileUp/> Import Data</Button>}</div>;
}

function DataEntry() {
  return <section className="overflow-hidden rounded-xl border bg-card enterprise-shadow">
    <div className="flex flex-col justify-between gap-3 border-b bg-secondary/40 px-5 py-4 sm:flex-row sm:items-center"><div><h2 className="flex items-center gap-2 font-bold"><ReceiptText className="size-5 text-primary"/> Boarder billing details</h2><p className="mt-1 text-xs text-muted-foreground">Complete the grouped fields below to prepare a billing record.</p></div><span className="w-fit rounded-full border bg-card px-3 py-1 text-[10px] font-bold uppercase tracking-wider text-muted-foreground">Entry template · 2026</span></div>
    <form className="p-5 sm:p-6" onSubmit={(e) => e.preventDefault()}>
      ...
      <div className="mt-6 flex flex-col-reverse justify-end gap-3 border-t pt-5 sm:flex-row"><Button type="reset" variant="subtle"><RotateCcw/> Clear form</Button><Button type="button" variant="enterprise"><ReceiptText/> Save entry</Button></div>
    </form>
  </section>;
}
...
function Reports() {
  return <section className="overflow-hidden rounded-xl border bg-card enterprise-shadow"><div className="border-b bg-secondary/30 p-5 sm:p-6"><div className="mb-4"><h2 className="font-bold">Find billing records</h2><p className="mt-1 text-xs text-muted-foreground">Use one or more fields to narrow the report view.</p></div><div className="grid gap-4 md:grid-cols-3 xl:grid-cols-[1fr_1.2fr_1fr_auto_auto]"><Field label="Filter by ID"><Input className="h-10 bg-card" placeholder="e.g. CMS-22-0874"/></Field><Field label="Filter by Name"><Input className="h-10 bg-card" placeholder="Enter student name"/></Field><SelectField label="Filter by Category" value="All categories" options={['All categories','Regular','Scholarship','Self Finance']}/><Button variant="enterprise" className="mt-auto"><FileUp/> Import Data</Button><Button variant="subtle" className="mt-auto"><Download/> Export Report</Button></div></div><div className="flex items-center justify-between border-b px-5 py-4"><div><h3 className="font-bold">Boarder billing records</h3><p className="mt-1 text-xs text-muted-foreground">5 sample records · Updated 13 June 2026</p></div><span className="hidden rounded-full bg-secondary px-3 py-1 text-xs font-bold text-secondary-foreground sm:inline-flex">All categories</span></div><div className="overflow-x-auto"><table className="w-full min-w-[900px] text-left text-sm"><thead className="bg-muted/70 text-[10px] uppercase tracking-wider text-muted-foreground"><tr>{['CMS ID','Student Name','Total Bill','Paid Amount','Balance','Status','Deposit Date','Actions'].map(h => <th className="px-5 py-3 font-extrabold" key={h}>{h}</th>)}</tr></thead><tbody className="divide-y">{records.map(row => <tr key={row[0]} className="transition-colors hover:bg-muted/40"><td className="px-5 py-4 font-mono text-xs font-bold text-primary">{row[0]}</td><td className="px-5 py-4 font-bold">{row[1]}</td>{row.slice(2,5).map((value,index) => <td key={index} className="px-5 py-4 tabular-nums">PKR {value}</td>)}<td className="px-5 py-4"><Status status={row[5]}/></td><td className="px-5 py-4 text-muted-foreground">{row[6]}</td><td className="px-5 py-4"><div className="flex gap-1"><Button variant="ghost" size="icon" title="View"><Eye/></Button><Button variant="ghost" size="icon" title="Edit"><Pencil/></Button><Button variant="ghost" size="icon" className="text-destructive" title="Delete"><Trash2/></Button></div></td></tr>)}</tbody></table></div><div className="flex items-center justify-between border-t px-5 py-4 text-xs text-muted-foreground"><span>Showing 5 records</span><div className="flex gap-2"><Button variant="subtle" size="sm" disabled>Previous</Button><Button variant="subtle" size="sm">Next</Button></div></div></section>;
}
